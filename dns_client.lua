...
local function resolve(qname, r_opts, dnsCacheOnly, try_list)
  ...
  -- log(ERR, PREFIX, "qname: ",qname, " qtype: ", qtype," opts: ", json.encode(r_opts), " try_list: ", json.encode(try_list), " cached records: ", json.encode(records))
  if records then
    if try_list then
      -- check for recursion
      if try_list["(short)"..qname..":"..tostring(qtype)] then
        ...
        log(ERR, PREFIX, "err: ", err, " try_list: ", json.encode(try_list) )
  ...
  log(ERR, PREFIX, "qname: ", qname,   " try_list: ", json.encode(try_list), " hostnametype: ", name_type)
  ...
  for try_name, try_type in search_iter(qname, qtype) do
    ...
    elseif #records == 0 then
      ...
      if records then
        log(ERR, PREFIX, "qname: ", qname,  " try_list: ", json.encode(try_list), " dnsCacheOnly: ", dnsCacheOnly,"records: ",json.encode(records))
        -- we have a result
